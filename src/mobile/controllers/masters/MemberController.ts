...
const getData = async (req: Request<{}, {}, {}, MemberQueryInterface>, res: Response) => {
    try {
        const result = await MemberService.getMembers(req.query, res.locals.userId, res.locals.level);
...
const postData = async (req: Request, res: Response) => {
    try {
        const result = await MemberService.createMember(req.body, res.locals.userId, res.locals.level);
        res.status(200).json({ status: true, ...result });
    } catch (error) {
        handleErrorMessage(res, error);
    }
...
const getSelect = async (req: Request, res: Response) => {
    try {
        const result = await MemberService.getMembersForSelect(req.query.name as string | undefined, res.locals.userId, res.locals.level);
        res.status(200).json({ status: true, ...result });
    } catch (error) {
        handleErrorMessage(res, error);
    }
