...
export const getMembers = async (query: MemberQueryInterface, userId: string, userLevel: string) => {
    ...
    return { message: "successful in getting Member data", data: { member: data, info: { page, limit: take, total } } };
};

export const createMember = async (memberData: any, userId: string, userLevel: string) => {
    const owner: any = await getOwnerId(userId, userLevel);
...
export const updateMember = async (id: string, memberData: any) => {
    await Model.members.update({ where: { id }, data: { id: memberData.id, name: memberData.name, address: memberData.address, level: parseInt(memberData.level + "") } });
...
export const getMembersForSelect = async (name: string | undefined, userId: string, userLevel: string) => {
    const owner: any = await getOwnerId(userId, userLevel);
    let filter: any = {};
    if (name) filter = { ...filter, name: { contains: name } };
    const data = await Model.members.findMany({ where: { ...filter, ownerId: owner.id }, take: 10 });
    const dataOption = [{ value: "", label: "" }, ...data.map(value => ({ key: value.id, value: value.name }))];
    return { message: "successfully in get Member data", data: { member: dataOption } };
};
