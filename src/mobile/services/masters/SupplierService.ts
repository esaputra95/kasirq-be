...
export const getSuppliers = async (query: SupplierQueryInterface, userId: string, userLevel: string) => {
    ...
    return { message: "successful in getting Supplier data", data: { supplier: data, info: { page, limit: take, total } } };
};

export const createSupplier = async (supplierData: any, userId: string, userLevel: string) => {
    const ownerId: any = await getOwnerId(userId, userLevel);
    if (!ownerId.status) throw new ValidationError("Owner not found", 404, "owner");
...
export const updateSupplier = async (id: string, supplierData: any) => {
    await Model.suppliers.update({ where: { id }, data: { id: supplierData.id, address: supplierData.address, name: supplierData.name } });
...
export const getSupplierById = async (id: string) => {
    ...
    return { message: "successfully in get Supplier data", data: { Supplier: supplier } };
};

export const getSuppliersForSelect = async (name: string | undefined, userId: string, userLevel: string) => {
    const owner: any = await getOwnerId(userId, userLevel);
    let filter: any = {};
    if (name) filter = { ...filter, name: { contains: name } };
    const data = await Model.suppliers.findMany({ where: { ...filter, ownerId: owner.id }, take: 25 });
    const dataOption = data.map(value => ({ key: value.id, value: value.name }));
    return { message: "successfully in get Supplier data", data: { supplier: dataOption } };
};
