...
export const getAccounts = async (query: AccountQueryInterface, userId: string, userType: string) => {
    ...
    return { message: "successful in getting Account data", data: { Account: data, info: { page, limit: take, total } } };
};

export const createAccount = async (accountData: any, userId: string, userType: string) => {
...
export const updateAccount = async (id: string, accountData: any, userId: string, userType: string) => {
    const ownerId: any = await getOwnerId(userId, userType);
    if (!ownerId.status) throw new ValidationError("Owner not found", 404, "owner");
...
export const getAccountById = async (id: string) => {
    ...
    return { message: "successfully in get Account data", data: { Account: account } };
};

export const getAccountsForSelect = async (name: string | undefined, userId: string, userType: string) => {
    const owner: any = await getOwnerId(userId, userType);
    let filter: any = {};
    if (name) filter = { ...filter, name: { contains: name } };
    const data = await Model.account.findMany({ where: { ...filter, ownerId: owner?.id }, take: 10 });
    const dataOption = [{ value: "", label: "" }, ...data.map(value => ({ value: value.id, label: value.name }))];
    return { message: "successfully in get Accounts data", data: { Account: dataOption } };
};
