...
const modelData = async (req: Request, res: Response) => {
    try {
        ...
        if (body?.storeId) {
            ...
            storeIds = ownerStores.map((store) => store.id);
        }
        ...
        const data: any[] = await Model.$queryRaw`
            ...
            WHERE 
                sales.date BETWEEN ${moment(
                    body?.startDate + " 00:00:00"
                ).format()} AND ${moment(body?.endDate + " 23:59:59").format()} 
                AND sales.storeId IN (${Prisma.join(storeIds)})
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            totalMargin +=
                parseInt(data[index].sell ?? "0") -
                parseInt(data[index].capital ?? "0");
            newResponse = [
                ...newResponse,
                [
                    index + 1,
